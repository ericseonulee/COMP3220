...
begin
   ...
   All_Day := Seconds(Time_And_Date);     -- Seconds since 00:00:00AM
...
begin
   Natural_Temp := NATURAL(Start_Seed - 0.5); -- Subtract 0.5 because the type conversion rounds the result.
...
begin
   Temp := A * X_Initial + C;
   Natural_Temp := NATURAL(Temp - 0.5);     -- Subtract 0.5 because the type conversion rounds the result.
